function [camParaCalib, varargout] = sk_calib_Co_planar(Ximg, Xp3D, camParaknown, imgsize, opt_offset)
...
%
% syntax:
%	camParaCalib = sk_calib_Co_planar(Ximg, Xp3D, camParaknown, ...);
%	[camParaCalib, err_img] = sk_calib_Co_planar(Ximg, Xp3D, camParaknown, ...);
%
% inputs:
...
if( opt_offset == 0 )
    ...
    offset = fminsearch( @myfun, offset0);
end
        ...
        if sum(abs(Xp3D(:,1))) == 0
            ...
            for i=1:5
                if w(i,i) > minsv
                    ...
                    warning('sk_calib_Co_planar: singular value of A1 < minsv, probably A1 is singular');
                end
            end
            ...
            if xi*X(i)*yi*Y(i) > 0  % if the signs of xi and yi are consistent with X(i) and Y(i)
                ...
                warning('sk_calib_Co_planar: inconsistent signs of xi, yi and X(i), Y(i)');
            end
            ...
            for i = 1:2
                if w(i,i) > minsv
                    ...
                    warning('sk_calib_Co_planar: singular value of A2 < minsv, probably A2 is singular');
                end
            end
            f_eff = a(1);
            Tz = a(2);
            ...
            while ((iter< max_iter) && ((f_err > f_tol) || (Tz_err > Tz_tol)) )
                ...
                for i = 1:2
                    if w(i,i) > minsv
                        ...
                        txt = strcat('sk_calib_Co_planar: iter = ', num2str(iter), ': singular value of A3 < minsv, probably A2 is singular');
                        warning(txt);
                    end
                end
            ...
            for i=1:7
                if w(i,i) > minsv
                    ...
                    warning('sk_calib_Co_planar: singular value of A < minsv, probably A is singular');
                end
            end
...
camParaCalib.p2 = 0;
camParaCalib.p2star = 0;
Xproj = sk_calibProj(camParaCalib, Xp3D);
% camParaCalib.err_t = sqrt(sum(sum((Ximg - sk_calibProj(camParaCalib, Xp3D)).^2))/Np);
err_img = Xproj - Ximg;
